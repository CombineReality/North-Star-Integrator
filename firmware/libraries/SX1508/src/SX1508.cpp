/******************************************************************************
SparkFunSX1508.cpp
SparkFun SX1508 I/O Expander Library Source File
Jim Lindblom @ SparkFun Electronics
Original Creation Date: September 21, 2015
https://github.com/sparkfun/SparkFun_SX1508rduino_Library
...
SX1508::SX1508(byte address, byte resetPin, byte interruptPin, byte oscillatorPin)
{
...
byte SX1508::begin(byte address, byte resetPin)
{
	// Store the received parameters into member variables
	deviceAddress =  address;
	pinReset = resetPin;
...
byte SX1508::init(void)
{
	...
	testRegisters = readWord(REG_INTERRUPT_MASK);	// This should return 0xFF00
...
void SX1508::configForNorthStarButtons() {
	writeByte(REG_INPUT_DISABLE, 0b11000000);
	writeByte(REG_PULL_UP, 0b00111111);
	writeByte(REG_INTERRUPT_SOURCE, 0b00111111);
	writeByte(REG_SENSE_HIGH, 0b00001111);
	writeByte(REG_SENSE_LOW, 0b11111111);
	debounceTime(32);
	writeByte(REG_DEBOUNCE_ENABLE, 0b00111111);
}

byte SX1508::readData() {
	return readByte(REG_DATA);
}
...
void SX1508::pinDir(byte pin, byte inOut)
{
	...
	byte tempRegDir = readByte(REG_DIR);
	if (modeBit)	
		tempRegDir |= (1<<pin);
	else
		tempRegDir &= ~(1<<pin);
...
void SX1508::pinMode(byte pin, byte inOut)
{
	pinDir(pin, inOut);
}

void SX1508::writePin(byte pin, byte highLow)
{
	byte tempRegDir = readByte(REG_DIR);
	
	if ((0xFF^tempRegDir)&(1<<pin))	// If the pin is an output, write high/low
	{
		byte tempRegData = readByte(REG_DATA);
	...
	{
		byte tempPullUp = readByte(REG_PULL_UP);
		byte tempPullDown = readByte(REG_PULL_DOWN);
		
		if (highLow)	// if HIGH, do pull-up, disable pull-down
		{
			tempPullUp |= (1<<pin);
			tempPullDown &= ~(1<<pin);
			writeByte(REG_PULL_UP, tempPullUp);
			writeByte(REG_PULL_DOWN, tempPullDown);
		}
...
void SX1508::digitalWrite(byte pin, byte highLow)
{
	writePin(pin, highLow);
}

byte SX1508::readPin(byte pin)
{
	byte tempRegDir = readByte(REG_DIR);
...
byte SX1508::digitalRead(byte pin)
{
	return readPin(pin);
}

void SX1508::ledDriverInit(byte pin, byte freq /*= 1*/, bool log /*= false*/)
{
...
void SX1508::pwm(byte pin, byte iOn)
{
...
void SX1508::analogWrite(byte pin, byte iOn)
{
	pwm(pin, iOn);
}

void SX1508::blink(byte pin, unsigned long tOn, unsigned long tOff, byte onIntensity, byte offIntensity)
{
...
void SX1508::breathe(byte pin, unsigned long tOn, unsigned long tOff, unsigned long rise, unsigned long fall, byte onInt, byte offInt, bool log)
{
...
void SX1508::setupBlink(byte pin, byte tOn, byte tOff, byte onIntensity, byte offIntensity, byte tRise, byte tFall, bool log)
{
...
void SX1508::keypad(byte rows, byte columns, unsigned int sleepTime, byte scanTime, byte debounceTime)
{
	//unsigned int tempWord;
	byte tempByte;
	...
	tempByte = readWord(REG_DIR);
	for (int i=0; i<rows; i++)
		tempByte &= ~(1<<i);
	for (int i=4; i<(columns * 2); i++)
		tempByte |= (1<<i);
	writeByte(REG_DIR, tempByte);
	
	// Set regOpenDrain on 0:7:
	tempByte = readByte(REG_OPEN_DRAIN);
	for (int i=0; i<rows; i++)
		tempByte |= (1<<i);
	writeByte(REG_OPEN_DRAIN, tempByte);
	...
	scanTimeBits &= 0b11100;	// Scan time is bits 2:0
	tempByte = sleepTime | scanTimeBits;
	writeByte(REG_KEY_CONFIG, tempByte);
	
	// RegKeyConfig2 tells the SX1508 how many rows and columns we've got going
	rows = (rows - 1) & 0b11;	// 0 = off, 0b001 = 2 rows, 0b111 = 8 rows, etc.
	columns = (columns - 1) & 0b11;	// 0b000 = 1 column, ob111 = 8 columns, etc.
	writeByte(REG_KEY_CONFIG, (rows << 3) | columns);
}

byte SX1508::readKeypad()
{
	return readKeyData();
}

byte SX1508::readKeyData()
{
	return (0xFF ^ readWord(REG_KEY_DATA));
}

byte SX1508::getRow(byte keyData)
{
	byte rowData = byte(keyData & 0x0F);
...
byte SX1508::getCol(byte keyData)
{
	byte colData = byte(keyData & 0xF0);
...
void SX1508::debounceConfig(byte configValue)
{
...
void SX1508::debounceTime(byte time)
{
...
void SX1508::debounceEnable(byte pin)
{
	byte debounceEnable = readByte(REG_DEBOUNCE_ENABLE);
	debounceEnable |= (1<<pin);
	writeByte(REG_DEBOUNCE_ENABLE, debounceEnable);
}

void SX1508::debouncePin(byte pin)
{
	debounceEnable(pin);
}

void SX1508::debounceKeypad(byte time, byte numRows, byte numCols)
{
...
void SX1508::enableInterrupt(byte pin, byte riseFall)
{
	// Set REG_INTERRUPT_MASK
	byte tempByte = readByte(REG_INTERRUPT_MASK);
	unsigned int tempWord;
	...
	writeWord(senseRegister, tempByte);
}

byte SX1508::interruptSource(bool clear /* =true*/)
{
	byte intSource = readByte(REG_INTERRUPT_SOURCE);
	if (clear)
		writeByte(REG_INTERRUPT_SOURCE, 0xFF);	// Clear interrupts
	return intSource;
}

bool SX1508::checkInterrupt(int pin)
{
...
void SX1508::clock(byte oscSource, byte oscDivider, byte oscPinFunction, byte oscFreqOut)
{
	configClock(oscSource, oscPinFunction, oscFreqOut, oscDivider);
}

void SX1508::configClock(byte oscSource /*= 2*/, byte oscPinFunction /*= 0*/, byte oscFreqOut /*= 0*/, byte oscDivider /*= 1*/)
{
...
byte SX1508::calculateLEDTRegister(int ms)
{
	int regOn1, regOn2;
...
byte SX1508::calculateSlopeRegister(int ms, byte onIntensity, byte offIntensity)
{
...
byte SX1508::readByte(byte registerAddress)
{
	byte readValue;
...
unsigned int SX1508::readWord(byte registerAddress)
{
...
//	- No return value.
void SX1508::readBytes(byte firstRegisterAddress, byte * destination, byte length)
{
	byte readValue;
...
//	- No return value.
void SX1508::writeByte(byte registerAddress, byte writeValue)
{
	Wire.beginTransmission(deviceAddress);
	Wire.write(registerAddress);
...
//	- No return value.
void SX1508::writeWord(byte registerAddress, unsigned int writeValue)
{
	byte msb, lsb;
...
//	- no return value.
void SX1508::writeBytes(byte firstRegisterAddress, byte * writeArray, byte length)
{
	Wire.beginTransmission(deviceAddress);
	Wire.write(firstRegisterAddress);
	for (int i=0; i<length; i++)
