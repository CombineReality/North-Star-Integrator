...
const short FAN_CYCLE = 5000;
const short FAN_HALF_CYCLE = FAN_CYCLE / 2;
const short FAN_TICK = FAN_HALF_CYCLE / 255;
...
void fanCycle() {
  int timeInCycle = millis() - fanTimer;
  if (timeInCycle < FAN_HALF_CYCLE) {
    analogWrite(FAN, constrain(timeInCycle / FAN_TICK, 0, 255));
  }
  else {
    if (timeInCycle < FAN_CYCLE) {
      analogWrite(FAN, constrain(255 - ((timeInCycle - FAN_HALF_CYCLE) / FAN_TICK), 0, 255));
    }
    else {
      fanCycling = false;
    }
